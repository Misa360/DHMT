...
float Px[MAX1] = {-3, -2, 0, 1, 2, 3};
float Py[MAX1] = {1, 3, 1, 3, 1, 3};
float Pz[MAX1] = {0, 0, 0, 0, 0, 0};
//float Pz[MAX1] = {1,2,3,4,5,6};

int Pn = 6; // So dinh dieu khien
int degre = 3;

float echX[MAX2], echY[MAX2], echZ[MAX2]; // Cac diem tinh toan tren duong cong 
int nbEch = 5; 
...
void bsplinePoint(float Px[MAX1], float Py [MAX1], float Pz [MAX1], int m, int k, float t, float *x, float *y, float *z)
{
    int i;
    float coeff;
    ...
    for (i=0; i<=m; i++)
    {
        ...
        // Them vao danh sach cac diem be mat
        (*x) = (*x) + (coeff * Px[k+i]);
        (*y) = (*y) + (coeff * Py[k+i]);
        (*z) = (*z) + (coeff * Pz[k+i]);
    }
...
void calcBSplineCurve(float Px[MAX1], float Py[MAX1], float Pz[MAX1], int Pn, int m, float echX[MAX2], float echY[MAX2], float echZ[MAX2], int nb)
{
    int i;
    float x, y, z;
    int k;
    ...
    for (k=0; k<=Pn-m-1; k++)
    {
        for (i=0; i<(nb+1); i++)
        {
            bsplinePoint(Px, Py, Pz, m, k, ((float)i)/((float)nb), &x, &y, &z);
            echX[i+k*(nb+1)] = x;
            echY[i+k*(nb+1)] = y;
            echZ[i+k*(nb+1)] = z;
        }
...
void display(void)
{
	...
	drawBSplinePolygon();
    calcBSplineCurve(Px, Py, Pz, Pn, degre, echX, echY, echZ, nbEch);
